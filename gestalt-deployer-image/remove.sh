...
remove_gestalt_platform() {
  ...
  kubectl delete applications,daemonsets,replicasets,statefulsets,services,deployments,pods,rc,secrets,configmaps,pvc,ingresses \
  ...
  # kubectl -n $install_namespace delete application gestalt
}
